...
def clean_trade_data_by_new_type(): # 관세청 수출입 데이터 
    ...
    #품목별 수출입 -------- -------- -------- -------- -------- --------    
    folder = r"C:\Users\bong2\OneDrive\DataArchive\DB_기타_출입국,수출입\3.수출입통계\1.2_품목별\\"
    ...
    folder = r"C:\Users\bong2\OneDrive\DataArchive\DB_기타_출입국,수출입\3.수출입통계\1.3_국가별전체\\"
    all_files = glob(folder + "*.xls")
    df = pd.concat([pd.read_excel(file, skiprows=4)
                 for file in all_files], axis=0).drop_duplicates()
...
def clean_data_from_unipass_by_product():  # 별도로 다운받은 수출입실적 HS코드10자리 데이터 전처리
    '''
    다운받을때 마다 오류가 계속 발생해서 수작업으로 jupyter에서 진행후
    다운로드폴더에 취합된 '취합본'만 남겨놓고 이 코드 실행
    '''
    print('10자리 HS코드 전처리 시작')
    path = r"C:\Users\bong2\OneDrive\DataArchive\DB_기타_출입국,수출입\3.수출입통계\1.5_별도로 받은 파일\0_받은원본\\"
    ...
    code_map_df = conn_db.from_('Master_수출입품목', '신성질_HS코드품목연계').drop(columns=drop_cols)
    ...
    path = r"C:\Users\bong2\OneDrive\DataArchive\DB_기타_출입국,수출입\3.수출입통계\1.5_별도로 받은 파일\\"
    df.to_pickle(path+'HSCODE_10_취합본.pkl') 
    print('10자리 HS코드 전처리 완료후 저장') 

@helper.timer
def clean_hscode_file():  # 수출입 hs코드 신성질별 분류파일 정리용
    '''
...
@helper.timer
def istans_hs_code_table():  # istans에서 받은 코드표 정리
    ...
    df_istans_ksic = df.sort_values(by=['GRC Code', 'ISTANS Code']).reset_index(drop=True)
